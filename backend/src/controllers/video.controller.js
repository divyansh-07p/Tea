...
const getVideoById = asyncHandler(async (req, res) => {
    ...
    const videoresponse = await Video.findById(videoId).populate("owner", "username fullName avatar")
    if (!videoresponse) {
        throw new ApiErrors(404,"Video not found")
    }
    return res.status(200).json(new ApiResponse(200, videoresponse, "Video fetched successfully"))
})
...
const togglePublishStatus = asyncHandler(async (req, res) => {
    ...
    await videoresponse.save()
    return res.status(200).json(new ApiResponse(200, videoresponse, "Video publish status toggled successfully"))
})
